 ...
 * @returns
 */
export function encodeCommandUriWithTelemetry(
	webview: string,
	identifier: string,
	command: string,
	args?: any[]
	...
	return `command:${helperCommand}?${encodeURIComponent(
		JSON.stringify(wrappedArgs)
	)}`;
}

export function encodeExternalLinkWithTelemetry(
	webview: string,
	name: string,
	url: string
) {
	return encodeCommandUriWithTelemetry(webview, name, "java.helper.openUrl", [
		url,
	]);
}
