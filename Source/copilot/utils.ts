...
export const CLASS_KINDS: SymbolKind[] = [
	SymbolKind.Class,
 ...
 */
export async function getClassesContainedInRange(
...
export async function getSymbolsContainedInRange(
	range: Range | Selection,
	document: TextDocument,
): Promise<SymbolNode[]> {
	const symbols = await getSymbolsOfDocument(document);
 ...
 */
export async function getInnermostClassContainsRange(
	range: Range | Selection,
	document: TextDocument,
): Promise<SymbolNode> {
	const symbols = await getSymbolsOfDocument(document);
	return (
		symbols
			...
			.reverse()
			.filter((clazz) => clazz.range.contains(range))[0]
	);
}
 ...
 */
export async function getIntersectionSymbolsOfRange(
	range: Range | Selection,
	document: TextDocument,
): Promise<SymbolNode[]> {
	const symbols = await getSymbolsOfDocument(document);
	return symbols
		.filter(
			(symbol) =>
				...
				FIELD_KINDS.includes(symbol.kind),
		)
 ...
 */
export async function getSymbolsOfDocument(
	document: TextDocument,
): Promise<SymbolNode[]> {
	const stack = (
		(await commands.executeCommand<DocumentSymbol[]>(
...
export async function getTopLevelClassesOfDocument(
	document: TextDocument,
): Promise<SymbolNode[]> {
	const symbols =
		(await commands.executeCommand<DocumentSymbol[]>(
...
export async function getProjectJavaVersion(
	document: TextDocument,
): Promise<string> {
	...
	try {
		...
		throw new Error(
			`Failed to get Java version, please check if the project is loaded normally: ${e}`,
		);
	}
}
 ...
 * copied from vscode-extension-telemetry-wrapper, the only difference is we re-throw the error to the caller but the original one doesn't.
 */
export function fixedInstrumentSimpleOperation(
	operationName: string,
	cb: (...args: any[]) => any,
	thisArg?: any,
): (...args: any[]) => any {
	return fixedInstrumentOperation(
		operationName,
		async (_operationId, ...args) => await cb.apply(thisArg, args),
		thisArg /** unnecessary */,
	);
}
