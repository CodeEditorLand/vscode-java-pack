...
import {
	fixedInstrumentSimpleOperation,
	getClassesContainedInRange,
...
export default class InspectionCopilot extends Copilot {
	public static readonly FORMAT_CODE = (
    ...
    **You are expert at Java and promoting newer built-in features of Java.**
	...
	public constructor(
		...
		super([
			...
			LanguageModelChatMessage.User(
				InspectionCopilot.EXAMPLE_USER_MESSAGE,
			),
			LanguageModelChatMessage.Assistant(
				InspectionCopilot.EXAMPLE_ASSISTANT_MESSAGE,
			),
		]);
	}
			...
			if (inspections.length < 1) {
				...
				void window
					.showInformationMessage(
						`Inspected ${target}, and got ${inspections.length} suggestions.`,
						"Go to",
					)
					.then((selection) => {
						selection === "Go to" &&
							void Inspection.revealFirstLineOfInspection(
								inspections[0],
							);
					});
			}
	...
	public inspectCode(
		...
		const _doInspectCode: (
			code: string,
			context: ProjectContext,
		) => Promise<Inspection[]> = fixedInstrumentSimpleOperation(
	 ...
	 */
	private extractInspections(
		codeWithInspectionComments: string,
		codeLines: { originalLineIndex: number; content: string }[],
	): Inspection[] {
		...
		return inspections
			.filter((i) => i.problem.indicator.trim() !== "<null>")
			.sort(
				(a, b) =>
					a.problem.position.relativeLine -
					b.problem.position.relativeLine,
			);
	}
	...
	private extractCodeLines(
		...
		for (
			...
			if (
				...
				codeLines.push({
					content: originalLines[originalLineIndex],
					originalLineIndex,
				});
			}
