...
export function registerCommands(
	copilot: InspectionCopilot,
	renderer: DocumentRenderer,
) {
	instrumentOperationAsVsCodeCommand(
		COMMAND_INSPECT_CLASS,
		async (document: TextDocument, clazz: SymbolNode) => {
			try {
	...
	instrumentOperationAsVsCodeCommand(
		COMMAND_INSPECT_RANGE,
		async (document: TextDocument, range: Range | Selection) => {
			try {
	...
	instrumentOperationAsVsCodeCommand(
		...
		async (
			...
			if (inspection) {
				sendInfo(`${COMMAND_IGNORE_INSPECTIONS}.info`, {
					problem: inspection.problem.description,
