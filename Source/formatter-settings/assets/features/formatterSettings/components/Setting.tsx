...
const Setting = (): JSX.Element => {
	const profileSettings: JavaFormatterSetting[] = useSelector(
		(state: any) => state.formatterSettings.profileSettings
	);
	const vscodeSettings: JavaFormatterSetting[] = useSelector(
		(state: any) => state.formatterSettings.vscodeSettings
	);
	const activeCategory: Category = useSelector(
		(state: any) => state.formatterSettings.activeCategory
	);
	const detectIndentation: boolean = useSelector(
		(state: any) => state.formatterSettings.detectIndentation
	);
	...
	const generateSetting = (setting: JavaFormatterSetting) => {
		...
		switch (setting.valueKind as ValueKind) {
			case ValueKind.Boolean:
				const willBeOverriden =
					detectIndentation &&
					setting.id === VSCodeSettings.DETECT_INDENTATION;
				return (
					<div
						className="setting-section"
						key={`${setting.id}`}
						onClick={() => handleClick(setting.exampleKind)}>
						<Form.Check
							type="checkbox"
							className="pl-0"
							id={`${setting.id}`}>
							...
							<Form.Check.Label className="setting-section-description">
								...
								<div className="setting-section-description-checkbox">
									{setting.name}.
								</div>
							</Form.Check.Label>
							<br></br>
							<span className="warning">
								{willBeOverriden
									? "When enabled, the indentation settings will be overriden based on the file contents."
									: ""}
							</span>
						</Form.Check>
					</div>
			...
			case ValueKind.Enum:
				...
				for (const candidate of setting.candidates) {
					candidates.push(
						<Dropdown.Item
							key={`${setting.id}.${candidate}`}
							className="dropdown-item py-0 pl-1"
							onSelect={() => handleSelect(setting, candidate)}>
							{candidate}
						</Dropdown.Item>
					);
				}
				return (
					<div
						...
						key={`${setting.id}`}
						onClick={() => handleClick(setting.exampleKind)}>
						<span className="setting-section-description">
							{setting.name}.
						</span>
						<Dropdown className="mt-1">
							<Dropdown.Toggle
								className="dropdown-button flex-vertical-center text-left"
								disabled={readOnly}>
			...
			case ValueKind.Number:
				return (
					<div
						...
						key={`${setting.id}`}
						onClick={() => handleClick(setting.exampleKind)}>
						<Form.Label className="setting-section-description my-0">
							{setting.name}.
						</Form.Label>
						<Form.Control
							className="mt-1 pl-1"
	...
	return <div className="setting">{result}</div>;
};
