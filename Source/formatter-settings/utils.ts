...
export async function addDefaultProfile(
	context: vscode.ExtensionContext,
): Promise<void> {
	const defaultProfile: string = path.join(
...
export async function getAbsoluteTargetPath(
	context: vscode.ExtensionContext,
	fileName: string,
): Promise<string> {
	const workspaceFolders = vscode.workspace.workspaceFolders;
...
export function parseProfile(document: vscode.TextDocument): ProfileContent {
	...
	const documentDOM = new DOMParser({
		...
		errorHandler: (_level, msg) => {
			const bracketExp: RegExp = new RegExp(
				"\\[line:(\\d*),col:(\\d*)\\]",
				"g",
			);
	...
	for (let i = 0; i < profiles.length; i++) {
		if (
			!settingsProfileName ||
			settingsProfileName === profiles[i].getAttribute("name")
		) {
				...
				if (!id) {
					diagnostics.push(
						new vscode.Diagnostic(
							new vscode.Range(
								new vscode.Position(
									setting.lineNumber - 1,
									setting.columnNumber - 1,
								),
								new vscode.Position(
									setting.lineNumber - 1,
									setting.columnNumber -
										1 +
										settingContent.length,
								),
							),
							"The setting has no valid 'id' property.",
				...
				if (!value) {
					...
					diagnostics.push(
						new vscode.Diagnostic(
							...
							"The setting has no valid 'value' property.",
							vscode.DiagnosticSeverity.Error,
						),
					);
	...
	for (const setting of supportedProfileSettings.values()) {
		...
		if (webViewValue === undefined) {
			...
			diagnostics.push(
				new vscode.Diagnostic(
					elementRange,
					`"${value}" is not supported in id: "${setting.id}".`,
					vscode.DiagnosticSeverity.Error,
				),
			);
...
export async function downloadFile(settingsUrl: string): Promise<string> {
	try {
		...
		const answer = await vscode.window.showErrorMessage(
			`Failed to get the profile content from uri: ${settingsUrl}. Do you want to retry?`,
			"Retry",
			"Open Settings",
		);
