	...
	implements vscode.CustomTextEditorProvider
{
	...
	private profileElements: Map<string, DOMElement> = new Map<
		string,
		DOMElement
	>();
	...
	private settingsVersion: string =
		JavaConstants.CURRENT_FORMATTER_SETTINGS_VERSION;
	private checkedRequirement: boolean = false;
	private checkedProfileSettings: boolean = false;
	private diagnosticCollection: vscode.DiagnosticCollection =
		vscode.languages.createDiagnosticCollection();
	private settingsUrl: string | undefined = vscode.workspace
		.getConfiguration("java")
		.get<string>(JavaConstants.SETTINGS_URL_KEY);
		...
		vscode.workspace.onDidChangeConfiguration(async (e) => {
			if (
				e.affectsConfiguration(
					`java.${JavaConstants.SETTINGS_URL_KEY}`
				) ||
				e.affectsConfiguration(
					`java.${JavaConstants.SETTINGS_PROFILE_KEY}`
				)
			) {
				...
				this.webviewPanel &&
				(e.affectsConfiguration(VSCodeSettings.TAB_SIZE) ||
					e.affectsConfiguration(VSCodeSettings.INSERT_SPACES) ||
					e.affectsConfiguration(VSCodeSettings.DETECT_INDENTATION))
			) {
				await this.updateVSCodeSettings();
				this.format();
			}
			if (
				e.affectsConfiguration(`java.${JavaConstants.SETTINGS_URL_KEY}`)
			) {
				this.settingsUrl = vscode.workspace
					.getConfiguration("java")
					.get<string>(JavaConstants.SETTINGS_URL_KEY);
			...
			async (e: vscode.TextDocumentChangeEvent) => {
				if (
					...
					e.document.uri.toString() !==
						vscode.Uri.file(this.profilePath).toString()
				) {
					return;
				}
	...
	public async showFormatterSettingsEditor(): Promise<void> {
		if (
			this.webviewPanel ||
			!(await this.checkProfileSettings()) ||
			!this.settingsUrl
		) {
			return;
		}
		const filePath = this.readOnly
			? vscode.Uri.parse(this.settingsUrl).with({
					scheme: RemoteProfileProvider.scheme,
			  })
			: vscode.Uri.file(this.profilePath);
			...
			switch (e.command) {
				...
				case "onWillChangeSetting":
					const settingValue: string | undefined =
						FormatterConverter.webView2ProfileConvert(
							e.id,
							e.value.toString()
						);
					...
					if (SupportedSettings.indentationSettings.includes(e.id)) {
						...
						if (e.id === SupportedSettings.TABULATION_CHAR) {
							const targetValue =
								settingValue === "tab" ? false : true;
							await config.update(
								VSCodeSettings.INSERT_SPACES,
								targetValue,
								undefined,
								true
							);
				...
				case "onWillDownloadAndUse": {
					...
					if (!settingsUrl || !isRemote(settingsUrl)) {
						vscode.window
							.showErrorMessage(
								"The active formatter profile does not exist or is not remote, please check it in the Settings and try again.",
								"Open Settings"
							)
							.then((result) => {
	...
	private async checkRequirement(): Promise<boolean> {
		...
		if (!javaExt) {
			vscode.window.showErrorMessage(
				"The extension 'redhat.java' is not installed. Please install it and run this command again."
			);
		...
		if (
			compareVersions(
				javaExtVersion,
				JavaConstants.MINIMUM_JAVA_EXTENSION_VERSION
			) < 0
		) {
			vscode.window.showErrorMessage(
				`The extension version of 'redhat.java' is too stale. Please install at least ${JavaConstants.MINIMUM_JAVA_EXTENSION_VERSION} and run this command again.`
			);
			const err: Error = new Error(
	...
	private async parseProfileAndUpdate(
		...
		if (!content.isValid) {
			vscode.window
				.showErrorMessage(
					"The current profile is invalid, please check it in the Settings and try again.",
					"Open Settings"
				)
				.then((anwser) => {
		...
		if (this.webviewPanel) {
			this.profileElements =
				content.profileElements || this.profileElements;
			this.profileSettings =
				content.profileSettings || this.profileSettings;
			...
			if (content.supportedProfileSettings) {
				this.webviewPanel.webview.postMessage({
					...
					setting: Array.from(
						content.supportedProfileSettings.values()
					),
				});
			}
			await this.updateVSCodeSettings();
		...
		if (this.webviewPanel?.visible) {
			vscode.window
				.showInformationMessage(
					`Formatter Profile settings have been changed, do you want to reload this editor?`,
					"Yes",
					"No"
				)
				.then(async (result) => {
		...
		for (const setting of supportedVSCodeSettings.values()) {
			switch (setting.id) {
				case SupportedSettings.TABULATION_CHAR:
					setting.value =
						getVSCodeSetting(VSCodeSettings.INSERT_SPACES, true) ===
						false
							? "tab"
							: "space";
				...
				case SupportedSettings.TABULATION_SIZE:
					setting.value = String(
						getVSCodeSetting(VSCodeSettings.TAB_SIZE, 4)
					);
					this.profileSettings.set(setting.id, setting.value);
					break;
	...
	private async format(): Promise<void> {
		const content = await vscode.commands.executeCommand<string>(
			"java.execute.workspaceCommand",
			"java.edit.stringFormatting",
			Example.getExample(this.exampleKind),
	...
	private async modifyProfile(
		...
		if (!profileElement) {
			...
			const originalString: string =
				new XMLSerializer().serializeToString(cloneElement);
			...
			edit.insert(
				document.uri,
				new vscode.Position(
					cloneElement.lineNumber - 1,
					cloneElement.columnNumber - 1 + originalString.length
				),
				(document.eol === vscode.EndOfLine.LF ? "\n" : "\r\n") +
					" ".repeat(cloneElement.columnNumber - 1) +
					new XMLSerializer().serializeToString(cloneElement)
			);
			await vscode.workspace.applyEdit(edit);
		} else {
			// edit a current setting in the profile
			const originalSetting: string =
				new XMLSerializer().serializeToString(profileElement);
			...
			edit.replace(
				...
				new vscode.Range(
					new vscode.Position(
						profileElement.lineNumber - 1,
						profileElement.columnNumber - 1
					),
					new vscode.Position(
						profileElement.lineNumber - 1,
						profileElement.columnNumber - 1 + originalSetting.length
					)
				),
				new XMLSerializer().serializeToString(profileElement)
			);
		...
		await vscode.workspace
			...
			.update(
				...
				workspaceFolders?.length
					? vscode.workspace.asRelativePath(profilePath)
					: profilePath,
				!workspaceFolders?.length
			);
	...
	private checkProfileSettings = instrumentOperation(
		"java.formatter.checkProfileSetting",
		async (operationId: string) => {
			...
			if (!this.settingsUrl) {
				...
				await vscode.window
					.showInformationMessage(
						"No active Formatter Profile found, do you want to create a default one?",
						"Yes",
						"No"
					)
					.then((result) => {
				...
				this.checkedProfileSettings = await vscode.window
					.showInformationMessage(
						"The active formatter profile is remote, do you want to open it in read-only mode or download and use it locally?",
					...
					.then(async (result) => {
						if (result === "Open in read-only mode") {
							...
							remoteProfileProvider.setContent(
								this.settingsUrl!,
								content
							);
							this.readOnly = true;
				...
				if (!(await fse.pathExists(this.profilePath))) {
					...
					await vscode.window
						.showInformationMessage(
							"The active formatter profile does not exist, please check it in the Settings and try again.",
							"Open Settings",
						...
						.then((result) => {
							if (result === "Open Settings") {
								...
								result === "Generate a default profile"
							) {
								addDefaultProfile(this.context);
							}
						});
...
export function initFormatterSettingsEditorProvider(
	context: vscode.ExtensionContext
) {
	javaFormatterSettingsEditorProvider =
		new JavaFormatterSettingsEditorProvider(context);
	context.subscriptions.push(
		vscode.window.registerCustomEditorProvider(
			JavaFormatterSettingsEditorProvider.viewType,
			javaFormatterSettingsEditorProvider,
			{
				webviewOptions: {
