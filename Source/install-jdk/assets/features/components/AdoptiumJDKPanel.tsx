...
import {
	AdoptiumAsset,
	AdoptiumReleaseInfo,
} from "../../../../utils/adoptiumApi";
import { useAppDispatch, useAppSelector } from "../../app/hooks";
import {
	onWillDownloadTemurinJDK,
	onWillFetchAsset,
...
const AdoptiumJDKPanel = () => {
	...
	const versionElements = releases?.available_lts_releases
		.map((v: number) => {
			return {
				version: v,
				isLts: true,
			...
			<VSCodeButton onClick={() => downloadJDK(asset)}>
				<div className="btn-download">
					...
					<div className="asset-info">
						...
						<div>
							<span>{asset.release_name}</span>
							<span> | </span>
							<span>
								{bytes(asset.binary.package?.size || 0, {
									unitSeparator: " ",
								})}
							</span>
						</div>
	...
	return (
		<div>
			...
			<VSCodeRadioGroup
				...
				<VSCodeRadio defaultChecked checked>
					hotspot
				</VSCodeRadio>
			</VSCodeRadioGroup>
			{downloadPanel}
