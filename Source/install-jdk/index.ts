...
export function showInstallJdkWebview(
...
export function showInstallJdkWebviewBeside(
	context: vscode.ExtensionContext,
	_operationId?: string,
) {
	InstallJdkPage.createOrShow(context.extensionPath, {
...
export class InstallJdkViewSerializer implements vscode.WebviewPanelSerializer {
	...
	async deserializeWebviewPanel(
		...
		InstallJdkPage.createOrShow(this.context.extensionPath, {
			webviewPanel,
		});
	}
}
...
class InstallJdkPage {
	...
	private constructor(
		...
		this._panel.iconPath = {
			light: vscode.Uri.file(
				path.join(extensionPath, "caption.light.svg"),
			),
	...
	private async doFetchAsset(payload: { majorVersion: number }) {
		const asset = await latestCompatibleAsset(
	...
	private async doDownloadTemurinJDK(payload: { url: string }) {
		await openExternalLinkFromWebview(
			InstallJdkPage.viewType,
			"Download",
			payload.url,
		);
