...
async function initializeExtension(
	...
	context.subscriptions.push(
		vscode.window.registerWebviewPanelSerializer(
			...
			new ClassPathConfigurationViewSerializer()
		)
	);
	context.subscriptions.push(
		vscode.window.registerWebviewPanelSerializer(
			"java.installJdk",
			new InstallJdkViewSerializer(context)
		)
	);
	...
	if (
		config.get("firstView") !== HelpViewType.None &&
		context.globalState.get(KEY_SHOW_WHEN_USING_JAVA)
	) {
