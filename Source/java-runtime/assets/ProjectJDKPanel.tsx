...
export class ProjectJDKPanel extends React.Component<Props, State> {
	...
	render = () => {
		...
		const projectTypeHint = (projectType: ProjectType) => {
			switch (projectType) {
				case "Maven":
					return (
						<VSCodeButton
							onClick={() => this.showHint("Maven")}
							appearance="icon"
							title="For projects managed by build tools, Java version is specified in build scripts(e.g. pom.xml).">
				...
				case "Gradle":
					return (
						<VSCodeButton
							onClick={() => this.showHint("Gradle")}
							appearance="icon"
							title="For projects managed by build tools, Java version is specified in build scripts(e.g. build.gradle).">
							<span className="codicon codicon-info"></span>
						</VSCodeButton>
					);
				default:
					return (
						<VSCodeButton
							onClick={() => this.showHint("Others")}
							appearance="icon"
							title="For folders containing .java files, but not managed by build tools like Maven/Gradle, a default JDK is used.">
							<span className="codicon codicon-info"></span>
						</VSCodeButton>
					);
			}
		...
		const downloadJDKCommand = encodeCommandUriWithTelemetry(
			"java.runtime",
			"download",
			"java.installJdk",
		);
		return (
			<div className="container">
				...
				{projectEntries.length > 0 && (
					<p>
						Manage Java runtime for your projects. If you don't have
						a valid Java runtime, you can{" "}
						<a href={downloadJDKCommand}>download</a> one.
					</p>
				)}
				{projectEntries.length > 0 ? (
					<VSCodeDataGrid
						...
						<VSCodeDataGridRow
							rowType={DataGridRowTypes.stickyHeader}>
							<VSCodeDataGridCell gridColumn="1">
								Project Name
							</VSCodeDataGridCell>
							<VSCodeDataGridCell gridColumn="2">
					...
					<div>
						<p>
							No project detected yet. Please refresh later if
							Java extension is importing your projects.
						</p>
						<VSCodeButton onClick={this.refresh}>
							Refresh
							<span
								slot="start"
								className="codicon codicon-refresh"></span>
						</VSCodeButton>
					</div>
				)}
...
function hasBuildTool(p: ProjectRuntimeEntry) {
	return (
		p.projectType === ProjectType.Maven ||
		p.projectType === ProjectType.Gradle
	);
}
