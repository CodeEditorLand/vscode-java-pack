...
export class DefaultJDKSelector extends React.Component<Props, State> {
	...
	render = () => {
		...
		return (
			<div className="inline-flex">
				{isEditing ? (
					<select
						className="jdkDropdown"
						id="jdkDropdown"
						defaultValue={p.runtimePath}
						onChange={this.onSelectionChange}>
						{jdkEntries.map((jdk) => (
							<option key={jdk.name} value={jdk.fspath}>
								{jdk.majorVersion} - {jdk.name}
							</option>
						))}
					</select>
				) : (
	...
	onSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
		...
		const targetJdk = this.props.jdkEntries.find(
			(jdk) => jdk.fspath === value,
		);
		if (targetJdk) {
