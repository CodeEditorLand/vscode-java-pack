...
export async function resolveRequirements(): Promise<{
	...
	return new Promise(async (resolve, reject) => {
		...
		const javaRuntimes = await findRuntimes({
			checkJavac: true,
			withVersion: true,
			withTags: true,
		});
		...
		if (!toolingJre) {
			...
			// pick a default project JDK/JRE
			/**
...
async function findEmbeddedJRE(
	javaExtPath?: string,
