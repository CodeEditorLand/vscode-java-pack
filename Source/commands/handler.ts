...
import { findLatestReleaseNotes, getReleaseNotesEntries } from "../utils";
...
export async function createMavenProjectCmdHandler(
	...
	if (
		!(await validateAndRecommendExtension(
			"vscjava.vscode-maven",
			"Maven extension is recommended to help create Java projects and work with custom goals.",
			true,
		))
	) {
		return;
...
export async function createSpringBootProjectCmdHandler(
	_context: vscode.ExtensionContext,
) {
	if (
		!(await validateAndRecommendExtension(
...
export async function createQuarkusProjectCmdHandler(
	_context: vscode.ExtensionContext,
) {
	if (
		!(await validateAndRecommendExtension(
			"redhat.vscode-quarkus",
			"Quarkus Tools for Visual Studio Code is recommended to help create Quarkus projects and for an all-in-one Quarkus application development experience.",
			true,
		))
	) {
		return;
...
export async function createMicroProfileStarterProjectCmdHandler(
	_context: vscode.ExtensionContext,
) {
	if (
		!(await validateAndRecommendExtension(
			"microProfile-community.mp-starter-vscode-ext",
			"MicroProfile Starter for Visual Studio Code is recommended to generate starter projects for Eclipse MicroProfile.",
			true,
		))
	) {
		return;
...
export async function showExtensionCmdHandler(
	_context: vscode.ExtensionContext,
	operationId: string,
...
export async function installExtensionCmdHandler(
	_context: vscode.ExtensionContext,
	operationId: string,
	extensionName: string,
	displayName: string,
	...
	return vscode.window
		.withProgress(
			{
				...
				title: `Installing ${displayName || extensionName}...`,
			},
			(_progress) => {
		...
		.then(() => {
			vscode.window.showInformationMessage(
				`Successfully installed ${displayName || extensionName}.`,
			);
		});
}

export async function openUrlCmdHandler(
	_context: vscode.ExtensionContext,
	operationId: string,
	url: string,
) {
...
export async function toggleAwtDevelopmentHandler(
	context: vscode.ExtensionContext,
	_operationId: string,
	enable: boolean,
) {
