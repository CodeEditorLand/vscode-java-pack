...
> {
		...
		return (
			<Container fluid className="root">
				<Row className="mb-4 mt-5">
					<Col className="page-content text-center">
						{this.steps[step]}
					</Col>
				</Row>
						...
						<div className="page-indicator text-center">
							{this.steps.map((_elem, index) => (
								<span
									key={index}
									className={
										index === step ? "active dot" : "dot"
									}
									onClick={() => this.showStep(index)}></span>
							))}
						</div>
					</Col>
				</Row>
	...
	getContentPages = () => {
		...
		const openFolderCommand: string = encodeCommandUriWithTelemetry(
			WEBVIEW_ID,
			"open folder",
			supportedByNavigator("mac")
				? "workbench.action.files.openFileFolder"
				: "workbench.action.files.openFolder"
		);
		const showProjectExplorerCommand: string =
			encodeCommandUriWithTelemetry(
				WEBVIEW_ID,
				"show project explorer",
				"javaProjectExplorer.focus"
			);
		const showRunAndDebugViewCommand: string =
			encodeCommandUriWithTelemetry(
				WEBVIEW_ID,
				"show run and debug view",
				"workbench.view.debug"
			);
		const showTestExplorerViewCommand: string =
			encodeCommandUriWithTelemetry(
				WEBVIEW_ID,
				"show test explorer",
				"testExplorer.focus"
		...
		const content = [
			{
				...
				description: (
					<div>
						<a href={openFolderCommand}>Open a folder</a> containing
						your Java project for full features.
					</div>
				),
			...
			},
			{
				...
				description: (
					<div>
						Expand{" "}
						<a href={showProjectExplorerCommand}>
							Java Project Explorer
						</a>{" "}
						to view your project structure.
					</div>
				),
			...
			},
			{
				...
				description: (
					<div>
						Open{" "}
						<a href={showRunAndDebugViewCommand}>
							Run and Debug View
						</a>{" "}
			...
			},
			{
				...
				description: (
					<div>
						Use{" "}
						<a href={showTestExplorerViewCommand}>Testing View</a>{" "}
						to run unit tests.
					</div>
				),
		...
		return content.map(
			(elem) => (
				<div className="text-center">
					...
					<div>
						<a href="#" onClick={() => skipTourFrom(elem.title)}>
							skip
						</a>
					</div>
				</div>
	...
	getStartingPage = () => {
		return (
			<div>
				...
				<div>
					<a href="#" onClick={() => skipTourFrom("Starting Page")}>
						skip
					</a>
				</div>
			</div>
	...
	getEndingPage = () => {
		return (
			<div>
				...
				<div>
					<Button onClick={this.nextStep}>What's next?</Button>
				</div>
			</div>
		);
	};
