...
export async function showWelcomeWebviewBeside(
...
async function initializeWelcomeView(
	context: vscode.ExtensionContext,
	webviewPanel: vscode.WebviewPanel,
...
const setWelcomeVisibility = instrumentSimpleOperation(
	"setWelcomeVisibility",
	(context: vscode.ExtensionContext, visibility: boolean) => {
		context.globalState.update(KEY_SHOW_WHEN_USING_JAVA, visibility);
	},
);

const setFirstTimeRun = (
	context: vscode.ExtensionContext,
	firstTimeRun: boolean,
) => {
...
export const fetchInitProps = async (context: vscode.ExtensionContext) => {
	welcomeView?.webview.postMessage({
		...
		props: {
			showWhenUsingJava: context.globalState.get(
				KEY_SHOW_WHEN_USING_JAVA,
			),
			firstTimeRun:
				context.globalState.get(KEY_IS_WELCOME_PAGE_VIEWED) !== true,
