...
export async function logsForLatestSession(
	logFilepath: string
): Promise<string> {
	const content = await fs.promises.readFile(logFilepath, {
		encoding: "utf-8",
	});
...
export function collectErrorsSince(log: string, timestamp: number): LogEntry[] {
	return collectErrors(log).filter(
		(e) => e.timestamp && e.timestamp > timestamp
	);
}
