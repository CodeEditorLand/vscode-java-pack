...
import {
	collectErrors,
	collectErrorsSince,
	containsCorruptedException,
	isUnsavedWorkspace,
	logsForLatestSession,
	sessionMetadata,
} from "./logUtils";
	...
	public async sendStartupMetadata(remark?: string) {
		if (this.serverLogUri) {
			...
			sendInfo("", {
				...
				initializeAt: toElapsed(
					metadata.startAt,
					metadata.initializeAt,
				)!,
				initializedAt: toElapsed(
					metadata.startAt,
					metadata.initializedAt,
				)!,
				importGradleAt: toElapsed(
					metadata.startAt,
					metadata.importGradleAt,
				)!,
				importMavenAt: toElapsed(
					metadata.startAt,
					metadata.importMavenAt,
				)!,
				initJobFinishedAt: toElapsed(
					metadata.startAt,
					metadata.initJobFinishedAt,
				)!,
				buildJobsFinishedAt: toElapsed(
					metadata.startAt,
					metadata.buildJobsFinishedAt,
				)!,
			});
		}
	}
