...
async function checkJavaExtActivated(
	...
	javaExt.exports.onDidServerModeChange(async (mode: string) => {
		if (mode === "Hybrid") {
			// begin to start standard language server
			checkIfJavaServerCrashed(30 * 1000 /*ms*/);
		}

		if (mode === "Standard") {
			daemon.logWatcher.sendStartupMetadata(
				"jdtls standard server ready",
			);
...
async function traceLSPPerformance(javaExt: vscode.Extension<any>) {
	...
	const isTreatment =
		...
		(redHatTelemetryEnabled ||
			(await getExpService()?.getTreatmentVariableAsync(
	...
	javaExt.exports?.onWillRequestStart?.((traceEvent: any) => {
		lspUsageStats?.recordRequestStart(
			traceEvent.type,
	...
	javaExt.exports?.onDidRequestEnd?.((traceEvent: any) => {
		...
		if (traceEvent.duration > 5000) {
			...
			lspUsageStats?.record1STimeoutRequest(
				traceEvent.type,
				traceEvent.fromSyntaxServer,
			);
		}
		...
		if (traceEvent.error) {
			...
			lspUsageStats?.recordErrorRequest(
				traceEvent.type,
				traceEvent.fromSyntaxServer,
			);
		...
		if (INTERESTED_REQUESTS.has(traceEvent.type)) {
			...
			if (
				!traceEvent.resultLength &&
				traceEvent.type === "textDocument/completion" &&
				(traceEvent.data?.triggerKind === undefined ||
					traceEvent.data?.triggerCharacter === " ")
			) {
				return;
			}
			...
			sendInfo("", {
				...
				duration,
				resultsize:
					traceEvent.resultLength === undefined
						? ""
						: String(traceEvent.resultLength),
				javaversion: javaExtVersion,
				remark: sampling,
				data: redactDataProperties(traceEvent.data),
				fromSyntaxServer,
		...
		if (traceEvent.resultLength === 0) {
			lspUsageStats?.recordNoResultRequest(
				traceEvent.type,
				traceEvent.fromSyntaxServer,
			);
		}
...
function traceJavaSettingUsage(
	context: vscode.ExtensionContext,
...
class LSPUsageStats {
	...
	public constructor(
		readonly javaExtVersion: string,
		readonly sampling: string,
		readonly fromSyntaxServer: boolean = false,
	) {}
