...
export class ClientLogWatcher {
		...
		if (logs) {
			...
			for (const log of logs) {
				if (log.message?.startsWith("Use the JDK from")) {
					...
					if (
						Date.parse(log.timestamp) < this.logProcessedTimestamp
					) {
						continue;
					}
					...
					const missingJar =
						"Error opening zip file or JAR manifest missing"; // lombok especially
					if (
						logs.find((log) => log.message?.startsWith(missingJar))
					) {
						info.error = missingJar;
					}

					const crashLog = logs.find((log) =>
						log.message?.startsWith(
	...
	private async readLatestLogFile() {
		if (!this.javaExtensionRoot) {
			try {
				this.javaExtensionRoot = vscode.Uri.file(
					path.dirname(
