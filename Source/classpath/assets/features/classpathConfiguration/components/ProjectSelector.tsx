...
import {
	onClickGotoProjectConfiguration,
	onWillLoadProjectClasspath,
	WEBVIEW_ID,
} from "../../../utils";
import {
...
const ProjectSelector = (): JSX.Element | null => {
	const activeProjectIndex: number = useSelector(
		(state: any) => state.classpathConfig.activeProjectIndex
	);
	const projects: ProjectInfo[] = useSelector(
		(state: any) => state.classpathConfig.projects
	);
	...
	const projectSelections = projects.map((project, index) => {
		return (
			<Dropdown.Item
				...
				onSelect={() => handleActiveProjectChange(index)}>
				{project.name}
			</Dropdown.Item>
		);
	});
				...
				{(projectType === ProjectType.Gradle ||
					...
					<div className="mt-1">
						<span className="warning">
							Below settings are only editable for projects
							without build tools. For {projectType} project,
							please edit the{" "}
							<a
								href={encodeCommandUriWithTelemetry(
									WEBVIEW_ID,
									`classpath.open${projectType}Doc`,
									"java.helper.openUrl",
									[buildFileDocUrl]
								)}>
								entries
							...
							in{" "}
							<a href="" onClick={() => handleOpenBuildFile()}>
								{buildFile}
							</a>
							.
						</span>
