...
import {
	...
	onWillRemoveReferencedLibraries,
} from "../../../utils";
import { ProjectType } from "../../../../../utils/webview";

const ReferencedLibraries = (): JSX.Element => {
	const referencedLibraries: string[] = useSelector(
		(state: any) => state.classpathConfig.referencedLibraries
	);
	const projectType: ProjectType = useSelector(
		(state: any) => state.classpathConfig.projectType
	);
	...
	const onDidAddReferencedLibraries = (
		event: OnDidAddReferencedLibrariesEvent
	) => {
		const { data } = event;
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener("message", onDidAddReferencedLibraries);
	}, []);
			...
			<ListGroup.Item
				className={`${
					projectType !== ProjectType.UnmanagedFolder
						? "inactive"
						: ""
				} list-row-body py-0 pl-0`}>
				<span className="ml-1">
					<em>No referenced libraries are configured.</em>
				</span>
			</ListGroup.Item>
		);
	} else {
		referencedLibrariesSections = referencedLibraries.map(
			(library, index) => (
				<ListGroup.Item
					className={`${
						projectType !== ProjectType.UnmanagedFolder
							? "inactive"
							: ""
					} list-row-body flex-vertical-center py-0 pl-0`}
					key={library}>
					...
					{projectType === ProjectType.UnmanagedFolder && (
						<span className="scale-up float-right">
							<a onClick={() => handleRemove(index)}>
								<Icon
									className="codicon cursor-pointer"
									icon={closeIcon}
								/>
							</a>
						</span>
	...
	return (
		<div>
			...
			<ListGroup className="list mt-1">
				<ListGroup.Item className="list-row-header py-0 pl-0 pr-2">
