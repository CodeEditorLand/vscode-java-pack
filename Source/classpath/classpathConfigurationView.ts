...
export class ClassPathConfigurationViewSerializer
...
async function initializeWebview(
	...
	context.subscriptions.push(
		classpathConfigurationPanel.onDidDispose(
			(_e) => (classpathConfigurationPanel = undefined)
		)
	);
	...
	context.subscriptions.push(
		classpathConfigurationPanel.webview.onDidReceiveMessage(
			async (message) => {
	...
	classpathConfigurationPanel.webview.html = getHtmlForWebview(
		classpathConfigurationPanel.webview,
		context.asAbsolutePath("./out/assets/classpath/index.js")
	);
...
const listProjects = instrumentOperation(
	"classpath.listProjects",
...
const loadProjectClasspath = instrumentOperation(
...
const debounceLoadProjectClasspath = _.debounce(
	loadProjectClasspath,
	3000 /*ms*/
);

const addSourcePath = instrumentOperation(
	"classpath.addSourcePath",
...
const removeSourcePath = instrumentOperation(
	"classpath.removeSourcePath",
	(
		_operationId: string,
...
const setOutputPath = instrumentOperation(
	"classpath.setOutputPath",
	async (operationId: string, currentProjectRoot: vscode.Uri) => {
...
const addReferencedLibraries = instrumentOperation(
	"classpath.addReferencedLibraries",
	async (_operationId: string, currentProjectRoot: vscode.Uri) => {
...
const removeReferencedLibrary = instrumentOperation(
	"classpath.removeReferencedLibrary",
	async (
		_operationId: string,
		currentProjectRoot: vscode.Uri,
		removalFsPath: string
	) => {
		...
		const removedPaths = _.remove(setting.include, (include) => {
			if (path.isAbsolute(include)) {
				...
				return (
					include ===
					vscode.workspace.asRelativePath(removalFsPath, false)
				);
			}
		});
...
const gotoProjectConfigurationFile = instrumentOperation(
...
async function getProjectClasspathFromLS(
	...
	const response = await lsApi!.getProjectSettings(uri.toString(), queryKeys);
	...
	classpath.sourcePaths = classpath.sourcePaths
		.map((p) => {
	...
	classpath.referenceLibraries = classpath.referenceLibraries
		.map((p) => {
...
function getReferencedLibrariesSetting(): IReferencedLibraries {
	const setting = vscode.workspace
		...
		.get<string[] | Partial<IReferencedLibraries>>("referencedLibraries");
