...
class ProjectSettingView {
	private classpathRequestHandler: ClasspathRequestHandler | undefined =
		undefined;

	public async showProjectSettingsPage(
	...
	public async initializeWebview(
		...
		context.subscriptions.push(
			projectSettingsPanel.onDidDispose(
				(_e) => (projectSettingsPanel = undefined),
			),
		);
		...
		context.subscriptions.push(
			(this.classpathRequestHandler = new ClasspathRequestHandler(
				projectSettingsPanel.webview,
			)),
		...
		projectSettingsPanel.webview.html = this.getHtmlForWebview(
			projectSettingsPanel.webview,
			context.asAbsolutePath("./out/assets/project-settings/index.js"),
		);
	}

	private listProjects = instrumentOperation(
		"projectSettings.classpath.listProjects",
		...
		if (lsApi) {
			getExtensionContext().subscriptions.push(
				...
				lsApi.onDidClasspathUpdate((uri: vscode.Uri) => {
					this.classpathRequestHandler?.debounceLoadProjectClasspath(
						uri,
					);
				}),
			);
	...
	private executeCommand = instrumentOperation(
...
export class ProjectSettingsViewSerializer
