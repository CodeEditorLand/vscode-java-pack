...
} from "./compilerConfigurationViewSlice";
...
const CompilerConfigurationView = (): JSX.Element | null => {
	...
	const vmInstalls: VmInstall[] = useSelector(
		(state: any) => state.classpathConfig.data.vmInstalls,
	);
	const activeVmInstallPath: string = useSelector(
		(state: any) =>
			state.classpathConfig.data.activeVmInstallPath[activeProjectIndex],
	);
	const activeVmInstall: VmInstall | undefined = vmInstalls.find(
		(vmInstall: VmInstall) => vmInstall.path === activeVmInstallPath,
	);
	...
	const availableComplianceLevels: string[] = useSelector(
		(state: any) => state.compilerConfig.ui.availableComplianceLevels,
	);

	// Hide the preview checkbox if the current JDK version is not latest & preview flag is already disabled.
	const showPreviewFlag: boolean =
		!!(
			availableComplianceLevels.find((level) => {
				return Number(level) > currentJdkComplianceLevel;
			}) === undefined
		) || enablePreview;

	const showSourceTargetWarning: boolean =
		!useRelease &&
		sourceLevel !== "" &&
		targetLevel !== "" &&
		Number(sourceLevel) > Number(targetLevel);
	const showJdkLevelWarning: boolean =
		Number(complianceLevel) > currentJdkComplianceLevel ||
		...
		<div className="root">
			<div className="setting-section">
				<div>
					<VSCodeCheckbox
						checked={useRelease}
						onClick={onClickUseRelease}>
						Use '--release' option for cross-compilation (Java 9 and
						later)
					</VSCodeCheckbox>
				</div>
				<div>
					<VSCodeDataGrid gridTemplateColumns="40% 60%">
						<VSCodeDataGridRow
							className={useRelease ? "" : "invisible"}>
							<VSCodeDataGridCell
								className="flex-center pl-0 pr-0"
								gridColumn="1">
				...
				<div
					className={`mb-2 mt-2 ${showSourceTargetWarning ? "" : "invisible"}`}>
					<span className="setting-section-warning">
						Target compatibility must be equal or greater than
						source compatibility.
					</span>
				</div>
				<div
					className={`mb-2 mt-2 ${showJdkLevelWarning ? "" : "invisible"}`}>
					<span className="setting-section-warning">
						Please make sure to have a compatible JDK configured
						(currently {currentJdkComplianceLevel}). You can change
						the JDK under the{" "}
						<VSCodeLink href="" onClick={() => onClickChangeJdk()}>
							JDK Runtime
						</VSCodeLink>{" "}
						tab.
					</span>
				...
				<div className={showPreviewFlag ? "" : "invisible"}>
					<VSCodeCheckbox
						...
						onClick={onClickEnablePreview}>
						Enable preview features
					</VSCodeCheckbox>
				</div>
				<VSCodeDivider className="mt-3" />
				<h4 className="mb-3 mt-3">Class File Generation</h4>
				<div>
					<VSCodeCheckbox
						checked={generateDebugInfo}
						onClick={onClickGenerateDebugInfo}>
						Generate debugging information
					</VSCodeCheckbox>
				</div>
				<div>
					<VSCodeCheckbox
						checked={storeMethodParamNames}
						onClick={onClickStoreMethodParamNames}>
