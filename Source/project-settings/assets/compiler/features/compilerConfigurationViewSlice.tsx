...
export const compilerConfigurationViewSlice = createSlice({
	...
	reducers: {
		initializeCompilerData: (state, action) => {
			...
			state.data.effective.generateDebugInfo =
				Array(projectNum).fill(false);
			state.data.effective.storeMethodParamNames =
				Array(projectNum).fill(false);
		},
		updateAvailableComplianceLevels: (state, action) => {
			state.ui.availableComplianceLevels =
				action.payload.availableComplianceLevels;
		},
		updateCompilerSettings: (state, action) => {
			...
			if (action.payload.useRelease !== undefined) {
				state.data.useRelease[activeProjectIndex] =
					action.payload.useRelease;
			}
			if (action.payload.enablePreview !== undefined) {
				state.data.enablePreview[activeProjectIndex] =
					action.payload.enablePreview;
			}
			if (action.payload.complianceLevel !== undefined) {
				state.data.complianceLevel[activeProjectIndex] =
					action.payload.complianceLevel;
			}
			if (action.payload.sourceLevel !== undefined) {
				state.data.sourceLevel[activeProjectIndex] =
					action.payload.sourceLevel;
			}
			if (action.payload.targetLevel !== undefined) {
				state.data.targetLevel[activeProjectIndex] =
					action.payload.targetLevel;
			}
			if (action.payload.generateDebugInfo !== undefined) {
				state.data.generateDebugInfo[activeProjectIndex] =
					action.payload.generateDebugInfo;
			}
			if (action.payload.storeMethodParamNames !== undefined) {
				state.data.storeMethodParamNames[activeProjectIndex] =
					action.payload.storeMethodParamNames;
			}
		},
		flushCompilerSettingsToEffective: (state, action) => {
			const activeProjectIndex = action.payload.activeProjectIndex;
			state.data.effective.useRelease[activeProjectIndex] =
				state.data.useRelease[activeProjectIndex];
			state.data.effective.enablePreview[activeProjectIndex] =
				state.data.enablePreview[activeProjectIndex];
			state.data.effective.complianceLevel[activeProjectIndex] =
				state.data.complianceLevel[activeProjectIndex];
			state.data.effective.sourceLevel[activeProjectIndex] =
				state.data.sourceLevel[activeProjectIndex];
			state.data.effective.targetLevel[activeProjectIndex] =
				state.data.targetLevel[activeProjectIndex];
			state.data.effective.generateDebugInfo[activeProjectIndex] =
				state.data.generateDebugInfo[activeProjectIndex];
			state.data.effective.storeMethodParamNames[activeProjectIndex] =
				state.data.storeMethodParamNames[activeProjectIndex];
		},
	},
});
