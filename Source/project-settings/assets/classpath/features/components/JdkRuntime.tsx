...
const JdkRuntime = (): JSX.Element => {
	...
	const [optionDescription, setOptionDescription] = useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		...
		// to change its style.
		document
			.querySelector("#jdk-dropdown")
			?.shadowRoot?.querySelector(".listbox")
			?.setAttribute("style", "max-height: initial;");
	...
	const addNewJdk = (
		<VSCodeOption
			...
			id="add-new-jdk"
			onMouseEnter={() =>
				setOptionDescription("Select a JDK from the local file system.")
			}
			onMouseLeave={() => setOptionDescription(activeVmInstallPath)}
