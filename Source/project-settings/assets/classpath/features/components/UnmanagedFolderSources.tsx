...
const UnmanagedFolderSources = (): JSX.Element => {
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener("message", onDidUpdateSourceFolder);
	}, []);
	...
	if (sources.length === 0) {
		sourceSections = (
			<VSCodeDataGridRow
				className={`${projectType !== ProjectType.UnmanagedFolder ? "inactive" : ""} setting-section-grid-row`}>
				<span>
	...
	return (
		<div className="setting-section">
			<h4 className="mb-1 mt-1 pl-1">Source Paths</h4>
			...
			<VSCodeDataGrid>{sourceSections}</VSCodeDataGrid>
		</div>
	);
};
