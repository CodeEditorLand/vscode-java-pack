...
const Sources = (): JSX.Element => {
	...
	const [editingSourcePath, setEditingSourcePath] = useState<string | null>(
		null,
	);
	const [editingOutputPath, setEditingOutputPath] = useState<string | null>(
		defaultOutput,
	);
	...
	const handleOK = () => {
		const updatedSources: ClasspathEntry[] = sources.map(
			(source, index) => {
		...
		if (sources.length === 0) {
			return (
				<VSCodeDataGridRow className="setting-section-grid-row">
					<span>
						<em>No source paths are configured.</em>
					</span>
				</VSCodeDataGridRow>
			);
		} else {
	...
	const getEditRow = (id: string) => {
		return (
			<VSCodeDataGridRow
				className="setting-section-grid-row"
				id={id}
				key={id}>
				<VSCodeDataGridCell
					className="setting-section-grid-cell setting-section-grid-cell-editable"
					gridColumn="1">
					<VSCodeTextField
						...
						onInput={(e: any) =>
							setEditingSourcePath(e.target.value)
						}>
						<VSCodeButton
							slot="end"
							appearance="icon"
				...
				<VSCodeDataGridCell
					...
					<VSCodeTextField
						...
						onInput={(e: any) =>
							setEditingOutputPath(e.target.value)
						}>
						<VSCodeButton
							slot="end"
							appearance="icon"
		...
		if (editRow === index) {
			...
			return (
				<VSCodeDataGridRow
					className="setting-section-grid-row"
					id={`sources-${index}`}
					onMouseEnter={() => setHoveredRow(`sources-${index}`)}
					onMouseLeave={() => setHoveredRow(null)}
					key={source.path}>
					<VSCodeDataGridCell
						className="setting-section-grid-cell setting-section-grid-cell-left setting-section-grid-cell-readonly"
						gridColumn="1">
						{source.path}
					</VSCodeDataGridCell>
					<VSCodeDataGridCell
						className="setting-section-grid-cell setting-section-grid-cell-readonly"
						gridColumn="2">
						<span>{source.output || defaultOutput}</span>
						<div
							className={
								hoveredRow === `sources-${index}`
									? ""
									: "hidden"
							}>
							<VSCodeButton
			...
			<div className="setting-overflow-area">
				<VSCodeDataGrid
					...
					<VSCodeDataGridRow
						...
						rowType="header">
						<VSCodeDataGridCell
							className="setting-section-grid-cell"
							cellType="columnheader"
							gridColumn="1">
							<span className="setting-section-grid-row-header">
								Path
							</span>
						</VSCodeDataGridCell>
						<VSCodeDataGridCell
