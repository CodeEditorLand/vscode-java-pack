...
const Libraries = (): JSX.Element => {
	...
	const resolveLibPath = (entry: ClasspathEntry): string => {
		if (entry.kind === ClasspathEntryKind.Project) {
			...
			entry.attributes?.hasOwnProperty("gradle_used_by_scope") ||
			entry.attributes?.hasOwnProperty("gradle.buildServer")
		) {
	...
	const resolveMavenLibPath = (entry: ClasspathEntry): string => {
		if (
			entry.attributes?.["maven.groupId"] &&
			entry.attributes?.["maven.artifactId"] &&
			entry.attributes?.["maven.version"]
		) {
	...
	if (libraries.length === 0) {
		librariesSections = (
			<VSCodeDataGridRow className="setting-section-grid-row">
				<span>
					<em>No libraries are configured.</em>
				</span>
			</VSCodeDataGridRow>
		);
	} else {
		librariesSections = libraries.map((library, index) => (
			<VSCodeDataGridRow
				...
				id={`library-${index}`}
				onMouseEnter={() => setHoveredRow(`library-${index}`)}
				...
				<VSCodeDataGridCell
					...
					<div
						...
						<span
							className={`codicon ${library.kind === 2 ? "codicon-project" : "codicon-library"} mr-1`}></span>
	...
	return (
		<div className="setting-section">
			<div>
				<div
					id="list-actions"
					className="flex-center setting-list-actions">
					<VSCodeButton
				...
				<div className="setting-overflow-area">
					<VSCodeDataGrid>{librariesSections}</VSCodeDataGrid>
				</div>
			</div>
		</div>
	);
