...
export class CompilerRequestHandler implements vscode.Disposable {
	...
	private onWillGetAvailableComplianceLevels = instrumentOperation(
		"projectSettings.compiler.onWillGetAvailableComplianceLevels",
		async (_operationId: string): Promise<void> => {
			...
			if (jres) {
				this.webview.postMessage({
					...
					complianceLevels: jres.map((jre: string) =>
						jre.substring(jre.lastIndexOf("-") + 1),
					),
				});
			}
		},
	...
	private onWillGetCompilerSettings = instrumentOperation(
		"projectSettings.compiler.onWillGetCompilerSettings",
		async (_operationId: string, uri: string): Promise<void> => {
			const response: any = await vscode.commands.executeCommand<Object>(
			...
			this.webview.postMessage({
				...
				enablePreview:
					response?.[ENABLE_PREVIEW_FEATURES] === "enabled",
				...
				generateDebugInfo:
					response?.[ADD_VARIABlLE_ATTRIBUTE] === "generate" &&
					response?.[ADD_LINE_NUMBER_ATTRIBUTE] === "generate" &&
					response?.[ADD_SOURCE_FILE_NAME] === "generate",
				storeMethodParamNames:
					response?.[STORE_METHOD_PARAMETER_INFO] === "generate",
			});
		},
	);

	private onWillUpdateCompilerSettings = instrumentOperation(
		"projectSettings.compiler.onWillUpdateCompilerSettings",
		async (
			operationId: string,
			uri: string,
			useRelease: boolean,
			...
			const compilerSettings: Map<string, string> = new Map<
				string,
				string
			>();
			...
			sendInfo(operationId, {
				...
				arg: JSON.stringify([...compilerSettings]).replace(
					/org\.eclipse\.jdt\.core\.compiler\./g,
					"",
				),
			});
		},
