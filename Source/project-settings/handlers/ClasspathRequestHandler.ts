...
export class ClasspathRequestHandler implements vscode.Disposable {
	...
	private async handleClasspathPanelRequest(message: any): Promise<void> {
		switch (message.command) {
			...
			case "classpath.onWillAddSourcePathForUnmanagedFolder":
				await this.addSourcePathForUnmanagedFolder(
			...
			case "classpath.onWillUpdateClassPaths":
				await this.updateClassPaths(
					...
					message.projectType,
					message.sourcePaths,
					message.defaultOutputPath,
					message.vmInstallPath,
					message.libraries,
				);
			...
			case "classpath.onClickGotoProjectConfiguration":
				this.gotoProjectConfigurationFile(
					message.rootUri,
					message.projectType,
				);
	...
	private listVmInstalls = instrumentOperation(
	...
	private debounceListVmInstalls = _.debounce(
		this.listVmInstalls,
		3000 /*ms*/,
	);

	private loadProjectClasspath = instrumentOperation(
		"projectSettings.classpath.loadClasspath",
		async (operationId: string, uri: vscode.Uri) => {
			if (
				!this.currentProjectRoot ||
				path.relative(uri.fsPath, this.currentProjectRoot.fsPath)
			) {
				return;
			}
	...
	public debounceLoadProjectClasspath = _.debounce(
		this.loadProjectClasspath,
		3000 /*ms*/,
	);

	private async selectSourceFolderPath(
		currentProjectRoot: vscode.Uri,
	): Promise<string | undefined> {
		const sourceFolder: vscode.Uri[] | undefined =
	...
	private addSourcePathForUnmanagedFolder = instrumentOperation(
		"projectSettings.classpath.addSourcePathForUnmanagedFolder",
		async (_operationId: string, currentProjectRoot: vscode.Uri) => {
			const relativePath: string | undefined =
				await this.selectSourceFolderPath(currentProjectRoot);
			...
			const sourcePaths: string[] =
				vscode.workspace
					...
					.get<string[]>("project.sourcePaths") || [];

			if (sourcePaths.includes(relativePath)) {
				vscode.window.showInformationMessage(
					`The path ${relativePath} has already been a source path.`,
				);
	...
	private updateSourcePathsForUnmanagedFolder = instrumentOperation(
		"projectSettings.classpath.updateSourcePathsForUnmanagedFolder",
		async (
			_operationId: string,
			currentProjectRoot: vscode.Uri,
			sourcePaths: string[],
	...
	private selectFolder = instrumentOperation(
		"projectSettings.classpath.selectFolder",
		async (
			_operationId: string,
			currentProjectRoot: vscode.Uri,
			type: string,
	...
	private updateClassPaths = instrumentOperation(
		"projectSettings.classpath.updateClassPaths",
		async (
			_operationId: string,
			rootPath: string,
			...
			try {
				const currentProjectRoot: vscode.Uri =
					vscode.Uri.parse(rootPath);

				if (projectType === ProjectType.UnmanagedFolder) {
					this.updateSourcePathsForUnmanagedFolder(
						currentProjectRoot,
	...
	private selectOutputPath = instrumentOperation(
		"projectSettings.classpath.selectOutputPath",
		async (_operationId: string, currentProjectRoot: vscode.Uri) => {
			const outputFolder: vscode.Uri[] | undefined =
				await vscode.window.showOpenDialog({
					defaultUri: vscode.workspace.workspaceFolders?.[0].uri,
			...
			if (outputFolder) {
				...
				const outputRelativePath: string = path.relative(
					projectRootPath,
					outputFullPath,
				);
	...
	private setOutputPath = instrumentOperation(
		"projectSettings.classpath.setOutputPath",
		async (
			operationId: string,
			currentProjectRoot: vscode.Uri,
			...
			if (
				vscode.workspace
					.getConfiguration("java", currentProjectRoot)
					.get<string>("project.outputPath") === outputRelativePath
			) {
				return;
			}

			const outputFullPath = path.join(
				currentProjectRoot.fsPath,
				outputRelativePath,
			);
			...
			if ((await fse.readdir(outputFullPath)).length) {
				const choice: string | undefined =
					await vscode.window.showInformationMessage(
						`The contents in ${outputFullPath} will be removed, are you sure to continue?`,
						"Yes",
						"No",
					);
	...
	private addNewJdk = instrumentOperation(
		"projectSettings.classpath.addNewJdk",
		async (operationId: string, currentProjectRoot: vscode.Uri) => {
			const actionResult: Record<string, string> = {
				name: "classpath.configuration",
				kind: "add-new-jdk",
			};

			const javaHome: vscode.Uri[] | undefined =
				await vscode.window.showOpenDialog({
					defaultUri: vscode.workspace.workspaceFolders?.[0].uri,
			...
			const result: IJdkUpdateResult =
				await vscode.commands.executeCommand<IJdkUpdateResult>(
					"java.execute.workspaceCommand",
	...
	private changeJdk = instrumentOperation(
		"projectSettings.classpath.changeJdk",
		async (
			operationId: string,
			currentProjectRoot: vscode.Uri,
			jdkPath: string,
	...
	private selectLibraries = instrumentOperation(
		"projectSettings.classpath.selectLibraries",
		async (_operationId: string, currentProjectRoot: vscode.Uri) => {
			const jarFiles: vscode.Uri[] | undefined =
				await vscode.window.showOpenDialog({
					defaultUri: vscode.workspace.workspaceFolders?.[0].uri,
	...
	private updateUnmanagedFolderLibraries = instrumentOperation(
		"projectSettings.classpath.updateUnmanagedFolderLibraries",
		async (_operationId: string, jarFilePaths: string[]) => {
	...
	private gotoProjectConfigurationFile = instrumentOperation(
		"projectSettings.classpath.gotoProjectConfigurationFile",
		(operationId: string, rootUri: string, projectType: ProjectType) => {
	...
	private async getVmInstallsFromLS(): Promise<VmInstall[]> {
		...
		try {
			ret.push(
				...((await vscode.commands.executeCommand<VmInstall[]>(
					"java.execute.workspaceCommand",
	...
	private async getProjectClasspathFromLS(
		uri: vscode.Uri,
	): Promise<ClasspathComponent> {
		...
		const classpath: ClasspathComponent = {
			projectType: getProjectType(
				uri.fsPath,
				response[NATURE_IDS] as string[],
			),
			sourcePaths: await this.getSourceRoots(
				response[CLASSPATH_ENTRIES_KEY],
				uri,
			),
		...
		const outputRelativePath: string = path.relative(
			baseFsPath,
			classpath.defaultOutputPath,
		);

		if (!outputRelativePath.startsWith("..")) {
			classpath.defaultOutputPath = path.relative(
				baseFsPath,
				classpath.defaultOutputPath,
			);
		}
	...
	private getLibraries(classpathEntries: ClasspathEntry[]): ClasspathEntry[] {
		...
		for (const entry of classpathEntries) {
			if (
				entry.kind === ClasspathEntryKind.Source ||
...
const CLASSPATH_ENTRIES_KEY: string =
