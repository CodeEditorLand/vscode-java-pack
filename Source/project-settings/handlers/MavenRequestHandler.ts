...
export class MavenRequestHandler implements vscode.Disposable {
	...
	private onWillGetSelectedProfiles = instrumentOperation(
		"projectSettings.maven.onWillGetSelectedProfiles",
		async (_operationId: string, uri: any): Promise<void> => {
	...
	private onWillUpdateSelectProfiles = instrumentOperation(
		"projectSettings.maven.onWillUpdateSelectProfiles",
		async (
			_operationId: string,
			uri: any,
			...
			await vscode.commands.executeCommand(
				...
				uri,
				{ [M2E_SELECTED_PROFILES]: selectedProfiles },
			);
		},
	);
